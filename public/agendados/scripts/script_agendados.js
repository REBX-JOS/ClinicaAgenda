// --- Sesión, Navbar, y acceso protegido ---
document.addEventListener('DOMContentLoaded', function () {
  // 1. Control de sesión y visibilidad de menú (similar a perfil)
  fetch("/api/login.php", {
    method: "POST",
  ...
  .then(json => {
    ...
    // Bloquea y redirige si no hay sesión
    if (!user) {
      window.location.href = "/logIn/html/html_logIn.html";
      return;
    }

    // Ajusta visibilidad del menú
    ...
    inicializarAgendados(user);
  });

  // --- Lógica principal de agendados ---
  function inicializarAgendados(user) {
    ...
    // Delegar click en botón cancelar
    if (servicesList) {
